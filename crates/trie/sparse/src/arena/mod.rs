...
struct ArenaSparseSubtrie {
    ...
    /// This is the absolute depth minus `UPPER_TRIE_MAX_DEPTH`, computed via `checked_sub`
    /// from [`ArenaParallelSparseTrie::update_leaves_cache_depth`].
...
pub struct ArenaParallelSparseTrie {
    ...
    /// Minimum absolute trie depth at which nodes have their cached RLP eagerly computed
    /// during `update_leaves`. When `Some(d)`, dirty branch nodes at absolute depth >= `d`
    /// in subtries will be RLP-encoded as they are popped from the cursor stack, avoiding
    ...
    /// The subtrie-local threshold is computed as `d - UPPER_TRIE_MAX_DEPTH` via `checked_sub`.
    update_leaves_cache_depth: Option<usize>,
...
impl ArenaParallelSparseTrie {
    ...
    /// Sets the minimum absolute trie depth for eager RLP caching during `update_leaves`.
    ///
    /// When set, dirty branch nodes at absolute depth >= `depth` in subtries will have their
    ...
    fn take_or_create_cleared_subtrie(&mut self) -> Box<ArenaSparseSubtrie> {
        ...
        subtrie.update_leaves_cache_depth =
            self.update_leaves_cache_depth.and_then(|d| d.checked_sub(UPPER_TRIE_MAX_DEPTH));
        ...
        while update_idx < sorted.len() {
            ...
            match find_result {
                ...
                SeekResult::RevealedSubtrie => {
                    ...
                    // Force inline processing when all updates are removals that could
                    // empty this subtrie. Parallel processing of such subtries risks
                    // multiple siblings being emptied simultaneously, causing branch
                    // collapse to hit a blinded sibling in the upper trie.
                    let force_inline = {
                        let all_removals = subtrie_updates
                            .iter()
                            .all(|(_, _, u)| matches!(u, LeafUpdate::Changed(v) if v.is_empty()));
                        if all_removals {
                            let num_leaves = match &self.upper_arena[child_idx] {
                                ArenaSparseNode::Subtrie(s) => s.num_leaves,
                                _ => 0,
                            };
                            num_subtrie_updates as u64 >= num_leaves
                        } else {
                            false
                        }
                    };
