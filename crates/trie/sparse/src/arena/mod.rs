...
impl ArenaSparseSubtrie {
    ...
    fn prune(&mut self, retained_leaves: &[Nibbles]) -> usize {
        ...
        // TODO: If we allocate to the `old_count` does it mean we didn't shrink anything?
        let mut new_arena = SlotMap::with_capacity(old_count);
        ...
        while let Some((new_idx, node_path)) = queue.pop_front() {
            ...
            let children: SmallVec<[(usize, u8, Index); 16]> =
                BranchChildIter::new(b.state_mask)
            ...
            for (child_pos, nibble, old_child_idx) in children {
                ...
                let child_short_key = match &self.arena[old_child_idx] {
                    ...
                    other => unreachable!(
                        "subtrie prune: unexpected child node kind: {other:?}"
                    ),
                };
...
impl SparseTrie for ArenaParallelSparseTrie {
    ...
    fn shrink_nodes_to(&mut self, size: usize) {
        ...
        // cleared_subtrie it will be properly shrinked.
